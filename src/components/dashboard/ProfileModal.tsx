...
export default function ProfileModal({ open, onClose }: Props) {
  ...
  const handleOverlayClose = () => {
    ...
    resetSubmissionStatus(); // Always reset the submission state
  };
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 grid place-items-center bg-black/50 p-4 backdrop-blur-sm">
        {/* --- THE FIX IS HERE: `overflow-hidden` has been added --- */}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-grow min-h-0">
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  <div className="p-1 mx-auto bg-gray-100 rounded-full flex items-center w-full">
                    <button type="button" onClick={() => setActiveTab('profile')} className={`flex-1 py-2 text-sm font-medium rounded-full transition-all duration-200 ${activeTab === 'profile' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50'}`}>Profile</button>
                    <button type="button" onClick={() => setActiveTab('preferences')} className={`flex-1 py-2 text-sm font-medium rounded-full transition-all duration-200 ${activeTab === 'preferences' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50'}`}>Preferences</button>
                  </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <button type="button" onClick={() => setIsEditingName(!isEditingName)} className="text-sm text-emerald-600 hover:underline">{isEditingName ? "Cancel" : "Edit"}</button>
                        </div>
            ...
            <div className="flex items-center justify-center flex-shrink-0 gap-3 p-4 bg-white border-t border-gray-200">
              ...
              <button type="submit" disabled={saving} className={`flex items-center justify-center w-32 h-10 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-md transition-all duration-200 ease-in-out hover:bg-emerald-700 hover:shadow-lg hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-emerald-700 disabled:shadow-inner disabled:translate-y-0.5`}>
                {saving ? (<>
                  <svg className="w-5 h-5 -ml-1 mr-2 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                  <span>Saving...</span></>
                ) : ("Save Changes")}
              </button>
            </div>
